...
const FileUpload = ({ name, label, idFile, setIdFile, ...field }) => {
  ...
  const getFile = (id) => {
    axios
      .get(`http://phuongnamdts.com:5000/files/${id}`)
      .then((response) => {
  ...
  const uploadFile = async (e) => {
    ...
    // try {
    //   const res = await axios.post("http://phuongnamdts.com:5000/upload", formData);
    //   // console.log(res);
    // } catch (ex) {
    //   // console.log(ex);
    // }
  ...
  return (
    <Box sx={{ display: "flex", alignItems: "center" }}>
      ...
      {/* {filePath && (
        <a href={`http://phuongnamdts.com:5000/${filePath.path}`} download>
          {filePath.name}
        </a>
      )} */}
    </Box>
