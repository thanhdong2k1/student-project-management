...
const DeadlineManagement = () => {
  ...
  const handleClickAdd = () => {
    ...
    localStorage.setItem("idDeadlineManagement", "");
    localStorage.setItem("deadlineManagementEdit", "");
    navigate(`../${routes.admin.addDeadlineManagement}`);
  };
  const handleClickEdit = async (id) => {
    const action = await rows.filter((row) => row.id == id);
    // console.log(id, action);
    localStorage.setItem("status", "edit");
    localStorage.setItem("idDeadlineManagement", id);
    localStorage.setItem("deadlineManagementEdit", JSON.stringify(action[0]));
    navigate(`../${routes.admin.addDeadlineManagement}`);
  };
  const handleClickDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete?")) {
      await deadlineManagementApi.delete({ id }).then(() => {
  ...
  useEffect(() => {
    const getDeadlineManagement = async () => {
      const responseDeadlineManagement = await deadlineManagementApi.getAll();
  ...
  //   const getDeadlineManagement = async () => {
  //     const responseDeadlineManagement = await deadlineManagementApi.getAll();
  //     // console.log(responseDeadlineManagement);
  //     // const res = responseDeadlineManagement.json();
  //     const dataTable = responseDeadlineManagement.map((res) => {
  //       return {
  //         id: res.MaGiangVien,
